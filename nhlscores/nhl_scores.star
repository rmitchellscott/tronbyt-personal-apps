...
def main(config):
    ...
    liveOnly = config.bool("live_only", False)
    minutesBefore = int(config.get("minutes_before", "60"))
    minutesAfter = int(config.get("minutes_after", "30"))
    ...
    if liveOnly and len(scores) > 0:
        has_active_game = False
        for s in scores:
            if is_game_in_window(s, now, minutesBefore, minutesAfter, timezone):
    ...
    return schema.Schema(
        ...
        fields = [
            ...
            schema.Toggle(
                ...
                desc = "Only show app when games are live or within the time window.",
                icon = "eyeSlash",
                default = False,
            ),
            schema.Text(
                id = "minutes_before",
                name = "Minutes before game",
                desc = "Show app this many minutes before game starts.",
                icon = "clock",
                default = "60",
            ),
            schema.Text(
                id = "minutes_after",
                name = "Minutes after game",
                desc = "Keep showing app this many minutes after game ends.",
                icon = "clock",
                default = "30",
            ),
        ],
...
def is_game_in_window(game, now, minutes_before, minutes_after, timezone):
    ...
    if status == "pre":
        ...
        game_time = time.parse_time(game_time_str, format = "2006-01-02T15:04Z").in_location(timezone)
        time_until_game = game_time - now
        window = time.parse_duration("%dm" % minutes_before)
        if time_until_game <= window and time_until_game >= time.parse_duration("0s"):
            return True

    if status == "post":
        game_time_str = game["date"]
        game_time = time.parse_time(game_time_str, format = "2006-01-02T15:04Z").in_location(timezone)
        estimated_end = game_time + time.parse_duration("3h")
        time_since_end = now - estimated_end
        window = time.parse_duration("%dm" % minutes_after)
        if time_since_end <= window and time_since_end >= time.parse_duration("0s"):
            return True
